...
add_shortcode('editar_artigo', function () {
    ...
    if (!isset($_GET['post_id'])) return '<p>Artigo não encontrado.</p>';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['titulo'])) {
        ...
        if ($post_id) {
            ...
            // Links do YouTube (apaga antigos e insere novos)
    ...
    // Pré-preenche dados do artigo
    $titulo = esc_attr($post->post_title);//
    $resumo = esc_textarea($post->post_excerpt);//
    $conteudo = $post->post_content;

    // Tags e categorias
    $tags = implode(', ', wp_get_post_tags($post_id, ['fields' => 'names']));
    $categorias = get_categories(['hide_empty' => false, 'orderby' => 'name']);
    $categoria_atual = wp_get_post_categories($post_id)[0] ?? '';

    // Links YouTube
    $links_youtube = get_post_meta($post_id, 'youtube_link');

    // Capa atual
    $capa_url = get_the_post_thumbnail_url($post_id, 'medium');
    ...
    <form method="POST" enctype="multipart/form-data" class="form-artigo">
        ...
        <div class="grid">
            ...
            <textarea name="resumo" id="resumo" rows="2" maxlength="100" required><?= $resumo ?></textarea>
        </div>
        
        <!-- conteudo -->
        <div class="grid">
            <label for="conteudo">Conteúdo</label>
            <div id="editor" style="height: 400px;"><?= $conteudo ?></div>
        ...
        <div class="grid">
            ...
            <?php if ($capa_url): ?>
                <img src="<?= esc_url($capa_url) ?>" style="max-width: 100%; margin-bottom: 10px; border-radius: 6px;">
            <?php endif; ?>
       ...
       <div class="grid">
            ...
            <select name="categoria_id" id="categoria_id" required>
                ...
                <?php
                    function exibir_categorias($cats, $categoria_atual, $parent = 0, $prefixo = '') {
                         foreach ($cats as $cat) {
                            if ($cat->parent == $parent) {
                                $selected = ($cat->term_id == $categoria_atual) ? 'selected' : '';
                                 echo '<option value="' . esc_attr($cat->term_id) . '" ' . $selected . '>' . esc_html($prefixo . $cat->name) . '</option>';
                                // Chamada recursiva para subcategorias
                                exibir_categorias($cats, $categoria_atual, $cat->term_id, $prefixo . '- ');
                            }
                        }
                    }

                    exibir_categorias($categorias, $categoria_atual);
                    ?>
            </select>
        </div>
        ...
        <div class="grid">
            ...
            <input type="text" name="tags" id="tags" value="<?= esc_attr($tags) ?>" placeholder="Palavra-chave 1, Palavra-chave 2, Palavra-chave 3, ...">
         ...
         <div class="grid" style="border: 1px solid #ccc; border-radius: 8px; padding: 20px">
             <div style="display: flex; flex-direction: row;">
                <div style="margin-right: 20px">
                    <input type="checkbox" checked name="check" id="check" required style="width: 18px; height: 18px">
                </div>
