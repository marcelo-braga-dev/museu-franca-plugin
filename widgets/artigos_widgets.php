...
add_shortcode('widgets_artigos', function ($atts = []) {
    ...
    // id único para permitir múltiplos carrosséis na mesma página
    ...
    <style>
        ...
        .wa-track{display:grid;grid-auto-flow:column;grid-auto-columns:clamp(180px,26vw,180px);gap:16px;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;padding:6px 4px 12px;scroll-behavior:smooth}
        .wa-track::-webkit-scrollbar{height:10px}
        .wa-track::-webkit-scrollbar-thumb{background:#e5e7eb;border-radius:10px}
        .wa-slide{scroll-snap-align:start}
        .wa-nav{position:absolute;inset:0;pointer-events:none}
        .wa-arrow{position:absolute;top:50%;transform:translateY(-50%);width:42px;height:42px;border-radius:999px;border:1px solid #9E2B19;background:#9E2B19;display:grid;place-items:center;box-shadow:0 8px 18px rgba(2,6,23,.12);pointer-events:auto;cursor:pointer;font-size:18px;color:#fff}
        .wa-prev{left:-6px}.wa-next{right:-6px}
        .wa-arrow[disabled]{opacity:.45;cursor:not-allowed}
        .wa-card{position:relative;border:1px solid #e6e8eb;border-radius:14px;overflow:hidden;background:#fff;transition:transform .18s,box-shadow .18s,border-color .18s;box-shadow:0 6px 18px rgba(2,6,23,.06)}
        .wa-card:hover{transform:translateY(-2px);border-color:#dbe2ea;box-shadow:0 16px 36px rgba(2,6,23,.1)}
        .wa-thumb-wrap{position:relative;display:block}
        .wa-thumb{display:block;width:100%;aspect-ratio:4/3;object-fit:cover}
        .wa-icons{position:absolute;left:8px;bottom:8px;display:flex;gap:6px}
        .wa-icons span{width:26px;height:26px;border-radius:50%;background:#fff;color:#111;display:flex;align-items:center;justify-content:center;font-size:14px;box-shadow:0 2px 6px rgba(0,0,0,.25)}
        .wa-fav{position:absolute;top:8px;right:8px;z-index:5}
        .wa-fav .mp-fav-button{all:unset;display:inline-flex!important;align-items:center;justify-content:center;width:36px;height:36px;border-radius:10px;background:#1e3a8a;color:#fff!important;border:1px solid rgba(255,255,255,.2);box-shadow:0 6px 16px rgba(0,0,0,.18);cursor:pointer}
        .wa-fav .mp-fav-button i{font-size:18px;color:#fff!important}
        .wa-fav .mp-fav-button.is-fav{background:#e11d48;border-color:#e11d48}
        .wa-fav .mp-fav-button,.wa-fav .mp-fav-button *{line-height:0}
        .wa-body{padding:10px}
        .wa-title-card{--lines:2;--lh:1.35;font-size:12px;font-weight:600;margin:0 0 3px;color:#0f172a;display:-webkit-box;-webkit-box-orient:vertical;overflow:hidden;line-height:var(--lh);-webkit-line-clamp:var(--lines);line-clamp:var(--lines);min-height:calc(var(--lines)*var(--lh)*1em);max-height:calc(var(--lines)*var(--lh)*1em)}
        .wa-meta{font-size:11px;color:#6b7280}
        .wa-actions{padding:0 12px 12px;text-align:end}
        .wa-btn{display:inline-flex;align-items:center;gap:6px;font-weight:500;font-size:11px;border-radius:10px;background:#fff;color:#9E2B19;text-decoration:none}
        .wa-btn:hover{background:#f8fafc;border-color:#cbd5e1}
    </style>
    ...
    <div class="wa-carousel" id="<?= esc_attr($uid) ?>">
        <?php if ($title): ?>
            <div class="wa-title">
                ...
                <h5 style="margin:0;font-size:18px"><?= esc_html($title) ?></h5>
            </div>
        <?php endif; ?>

        <div class="wa-track" id="<?= esc_attr($uid) ?>_track" role="region" aria-roledescription="carousel" aria-label="<?= esc_attr($title ?: 'Conteúdos') ?>">
                    ...
                    <article class="wa-card">
                        <?php if ($thumb): ?>
                            <a href="<?= esc_url($link) ?>" class="wa-thumb-wrap" aria-label="<?= esc_attr($rtitle) ?>">
                                <img class="wa-thumb" src="<?= esc_url($thumb) ?>" alt="<?= esc_attr($rtitle) ?>" loading="lazy">
                                <div class="wa-icons">
                                    <?php if ($has_img): ?><span title="Imagem"><i class="fa-regular fa-image" style="color:#16a34a"></i></span><?php endif; ?>
                                    <?php if ($has_vid): ?><span title="Vídeo"><i class="fa-brands fa-youtube" style="color:#ef4444"></i></span><?php endif; ?>
                                    <?php if ($has_pdf): ?><span title="PDF"><i class="fa-regular fa-file-pdf" style="color:#b91c1c"></i></span><?php endif; ?>
                                    <?php if ($has_aud): ?><span title="Áudio"><i class="fa-solid fa-volume-high" style="color:#111827"></i></span><?php endif; ?>
                                </div>
                            </a>
                        <?php endif; ?>
        ...
        <div class="wa-nav" aria-hidden="false">
            <button class="wa-arrow wa-prev" id="<?= esc_attr($uid) ?>_prev" type="button" aria-label="Anterior"><i class="fa-solid fa-chevron-left"></i></button>
            <button class="wa-arrow wa-next" id="<?= esc_attr($uid) ?>_next" type="button" aria-label="Próximo"><i class="fa-solid fa-chevron-right"></i></button>
        </div>
    </div>
    ...
    <script>
        (function () {
            ...
            function canScrollLeft(){ return track.scrollLeft > 10; }
            function canScrollRight(){ return track.scrollLeft + track.clientWidth < track.scrollWidth - 10; }
            function updateArrows(){ if (prev) prev.disabled = !canScrollLeft(); if (next) next.disabled = !canScrollRight(); }
            function scrollAmount(){ return Math.max(track.clientWidth * 0.9, 260); }
