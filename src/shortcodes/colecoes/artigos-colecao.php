...
function mp_sc_artigos_colecao($atts = []) {
    ...
    <style>
        ...
        .mp-card-artigo{border:1px solid #e5e7eb;border-radius:12px;background:#fff;overflow:hidden;display:flex;flex-direction:column;gap:.6rem;}
        .mp-card-thumb img{width:100%;height:200px;object-fit:cover;display:block;}
        .mp-thumb-placeholder{width:100%;height:200px;display:grid;place-items:center;background:#f3f4f6;color:#6b7280;}
    ...
    <section class="mp-lista-artigos-colecao" aria-label="Artigos da coleção">
        <header class="mp-lista-header">
            <h2>Coleção: <?php echo esc_html($term->name); ?></h2>
            <?php if (!empty($term->description)): ?>
                <p class="mp-colecao-desc"><?php echo esc_html($term->description); ?></p>
            <?php endif; ?>
        </header>
            ...
            <div class="mp-grid-artigos">
                <?php while ($q->have_posts()): $q->the_post();
                    ...
                    <article class="mp-card-artigo">
                        <a class="mp-card-thumb" href="<?php echo esc_url(get_permalink($pid)); ?>">
                            <?php if ($thumb): ?>
                        ...
                        <div class="mp-card-meta">
                            <?php echo function_exists('mp_colecao_badges') ? mp_colecao_badges($pid) : ''; ?>
                        </div>
                    </article>
