...
function mp_sc_lista_colecoes($atts = []) {
    ...
    $terms = get_terms(['taxonomy' => 'colecao', 'hide_empty' => false]);
    ...
    <style>
        ...
        @media(max-width:800px){.mp-colecoes-grid{grid-template-columns:repeat(1,minmax(0,1fr)) !important;}}
        .mp-colecao-card{border:1px solid #e5e7eb;border-radius:12px;padding:1rem;background:#fff;display:flex;flex-direction:column;gap:.5rem;}
    ...
    <div class="mp-colecoes-grid cols-<?php echo (int)$cols; ?>">
        <?php foreach ($terms as $t):
            ...
            <article class="mp-colecao-card">
                ...
                <?php if ($a['show_count'] === 'yes'): ?>
                    <div class="mp-colecao-meta"><?php echo (int)$t->count; ?> artigo(s)</div>
                <?php endif; ?>
                <?php if (!empty($t->description)): ?>
                    <p class="mp-colecao-desc"><?php echo esc_html($t->description); ?></p>
                <?php endif; ?>
                <a class="mp-colecao-cta" href="<?php echo esc_url(get_term_link($t)); ?>">
