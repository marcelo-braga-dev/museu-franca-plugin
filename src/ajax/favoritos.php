...
if (!function_exists('mp_get_user_favorites')) {
    function mp_get_user_favorites($user_id = 0) {
        ...
        $ids = get_user_meta($user_id, MP_META_FAVORITOS, true);
        if (!is_array($ids)) $ids = [];
...
if (!function_exists('mp_toggle_favorite')) {
    function mp_toggle_favorite($post_id, $user_id = 0) {
        ...
        if (in_array($post_id, $favoritos, true)) {
            ...
            update_user_meta($user_id, MP_META_FAVORITOS, $favoritos);
...
.mp-fav-button i{ font-size:20px; color:var(--mp-fav-gray) !important; transition: transform .20s ease, color .2s ease; }
.mp-fav-button:hover{ border-color:var(--mp-fav-red); background:var(--mp-fav-hover-bg); box-shadow:0 6px 16px var(--mp-fav-shadow); transform:translateY(-2px); }
.mp-fav-button:active{ transform:translateY(0); }
.mp-fav-button:focus-visible{ outline: 3px solid color-mix(in srgb, var(--mp-fav-red) 40%, transparent); outline-offset: 2px; }
.mp-fav-button.is-fav{ border-color:var(--mp-fav-red); background:var(--mp-fav-active-bg); color:var(--mp-fav-red); }
.mp-fav-button.is-fav i{ color:var(--mp-fav-red) !important; }
.mp-fav-button.is-loading, .mp-fav-button:disabled{ opacity:.7; cursor:progress; pointer-events:none; }
.mp-fav-guest{ text-decoration:none; }
.mp-fav--icon{ width:40px; height:40px; padding:0; border-radius:9999px; background:#ffffff !important; border:1px solid #e5e7eb; box-shadow:0 6px 16px rgba(0,0,0,.12); }
.mp-fav--icon:hover{ background:#ffffff !important; border-color:var(--mp-fav-red); box-shadow:0 10px 22px rgba(220,38,38,.15); transform:translateY(-2px); }
.mp-fav--icon.is-fav{ background:#ffffff !important; border-color:var(--mp-fav-red); }
.mp-fav--icon i{ font-size:18px; }
@media (max-width: 480px){ .mp-fav-button{ padding:8px 12px; font-size:14px; } .mp-fav-button i{ font-size:18px; } .mp-fav--icon{ width:38px; height:38px; } }
